...
class SaveInterval extends Action
{
    /**
     * Core registry
     *
    ...
    public function execute()
    {
        ...
        if($okayVaidate && $intervalTime != '')
        {
            try{
                ...
                $htmlCalendarForm = $this->_view->getLayout()->createBlock('Magebay\Bookingsystem\Block\Adminhtml\Calendars')->setData($dataSend)->setTemplate('Magebay_Bookingsystem::catalog/product/calendars/interval-items.phtml')->toHtml();
                $status = 'success';
            }
            catch (\Exception $e)
            {
