...
class Reportproduct extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if($isseller){	
    		...
    		$resultPageFactory->getConfig()->getTitle()->set(__('Report Product'));	
    		if($breadcrumbs = $resultPageFactory->getLayout()->getBlock('breadcrumbs')){
                ...
                $breadcrumbs->addCrumb('market_menu_reportproduct_detail',
                    [
                        'label' => __('Report Product'),
                        'title' => __('Report Product')
                    ]
                ); 
            }
