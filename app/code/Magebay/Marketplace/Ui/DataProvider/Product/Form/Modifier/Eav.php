...
class Eav extends AbstractModifier
{
    ...
    public function modifyMeta(array $meta)
    {
        ...
        foreach ($this->getGroups() as $groupCode => $group) {
			//if($groupCode=='container_price') continue;
        ...
        if($moduleManager->isEnabled('Magebay_ProductAttributeManagement')){
            foreach($meta as $key=>$data){
                foreach($data['children'] as $key1 => $val){
                    $k = explode('container_',$key1);
                    $data1 = \Magento\Framework\App\ObjectManager::getInstance()->create('Magebay\ProductAttributeManagement\Model\ProductAttributeManagement')->getCollection();
                    foreach($data1->getData() as $dt){
                        if(in_array($k[1],$dt)){
                            if($dt['status'] == 0){
                                unset($meta[$key]['children']['container_'.$k[1]]);
                            }
                        }
                    }                        
                }
    ...
    private function getGroups()
    {
        if (!$this->attributeGroups) {
            ...
            foreach ($attributeGroupSearchResult->getItems() as $group) {				
				if($group->getAttributeGroupCode() == 'product-details' || $group->getAttributeGroupCode()=='advanced-pricing' || $group->getAttributeGroupCode()=='content' || $group->getAttributeGroupCode()=='autosettings' || $group->getAttributeGroupCode()=='search-engine-optimization' )
				//if($group->getAttributeGroupCode() == 'product-details')
                $this->attributeGroups[$this->calculateGroupCode($group)] = $group;
				//$__group[] = $group->getAttributeGroupCode();
            }
        }		
    ...
    { 		
		...
		if($formElement == 'select' && $attribute->getFrontendInput() == 'select'){
			$arrMeta['component'] = 'Magento_Ui/js/form/element/select';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';
			$arrMeta['elementTmpl'] = 'Magebay_Marketplace/form/element/select';			
		}else if($formElement == 'input' && (($attribute->getFrontendInput()=='text' && $attribute->getAttributeCode() != 'tier_price') || $attribute->getFrontendInput()=='weight') && $attribute->getFrontendInput() !='sku'){
			//$arrMeta['component'] = 'Magento_Ui/js/form/element/abstract';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';
			//if($attribute->getFrontendInput()=='weight'){
				$arrMeta['elementTmpl'] = 'Magebay_Marketplace/form/element/input';			
			//}
		}else if($attribute->getFrontendInput()=='price'){			
			//$arrMeta['dataType'] = 'text';
			$arrMeta['component'] = 'Magento_Ui/js/form/element/abstract';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';
			$arrMeta['elementTmpl'] = 'Magebay_Marketplace/form/element/input';			
			//$arrMeta['elementTmpl'] = 'Magebay_Marketplace/form/field';
		}else if($formElement=='date'){
			$arrMeta['component'] = 'Magento_Ui/js/form/element/date';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';						
			//$arrMeta['elementTmpl'] = 'Magebay_Marketplace/form/element/date';
		}else if($formElement == 'checkbox'){
			$arrMeta['component'] = 'Magento_Ui/js/form/element/single-checkbox';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';			
			//$arrMeta['elementTmpl'] = 'Magento_Ui/form/element/field';						
		}else if($formElement == 'multiselect'){
			$arrMeta['component'] = 'Magento_Ui/js/form/element/multiselect';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';						
		}else if($formElement == 'textarea'){
			$arrMeta['component'] = 'Magento_Ui/js/form/element/textarea';
			$arrMeta['template'] = 'Magebay_Marketplace/form/field';				
		}
        $meta = $this->arrayManager->set($configPath, [], $arrMeta);
        ...
        if ($attribute->getIsWysiwygEnabled()) {
            $containerMeta = $this->arrayManager->merge(
                ...
                [
					...
					'template' => 'Magebay_Marketplace/group/group',
					'fieldTemplate' => 'Magebay_Marketplace/form/field',													
                ]
            );
        }
    ...
    private function customizeWysiwyg(ProductAttributeInterface $attribute, array $meta)
    {
        ...
        $meta['arguments']['data']['config']['formElement'] = 'wysiwyg';
        $meta['arguments']['data']['config']['component'] = 'Magebay_Marketplace/js/form/element/wysiwyg';
        $meta['arguments']['data']['config']['wysiwyg'] = true;
		if($meta['arguments']['data']['config']['code'] == 'description'){
			$meta['arguments']['data']['config']['content'] = '<textarea data-ui-id="form-element-description" class=" textarea admin__control-textarea" cols="15" rows="20" name="description" id="magebaymarketproduct_form_description"></textarea>'            
			.'<script type="text/javascript">
			//var wysiwygmagebaymarketproduct_form_description, editorFormValidationHandler;
                //
                window.tinyMCE_GZ = window.tinyMCE_GZ || {}; window.tinyMCE_GZ.loaded = true;
				require(["mage/adminhtml/wysiwyg/tiny_mce/setup"], function(){
  //(function($) {$.mage.translate.add({"Insert Image...":"Insert Image...","Insert Media...":"Insert Media...","Insert File...":"Insert File..."})})(jQuery);
wysiwygmagebaymarketproduct_form_description = new tinyMceWysiwygSetup("magebaymarketproduct_form_description", {"enabled":true,"hidden":false,"use_container":false,"add_variables":false,"add_widgets":false,"no_display":false,"encode_directives":true,"baseStaticUrl":"","baseStaticDefaultUrl":"","directives_url":"","popup_css":"","content_css":"","width":"100%","height":"500px","plugins":[],"directives_url_quoted":"","add_images":true,"files_browser_window_url":"","files_browser_window_width":"1000","files_browser_window_height":"600"});wysiwygmagebaymarketproduct_form_description.setup("exact");
                    //editorFormValidationHandler = wysiwygmagebaymarketproduct_form_description.onFormValidation.bind(wysiwygmagebaymarketproduct_form_description);
                   // Event.observe("toggleproduct_form_description", "click", wysiwygmagebaymarketproduct_form_description.toggle.bind(wysiwygmagebaymarketproduct_form_description));
                    //varienGlobalEvents.attachEventHandler("formSubmit", editorFormValidationHandler);
                    //varienGlobalEvents.clearEventHandlers("open_browser_callback");
                    //varienGlobalEvents.attachEventHandler("open_browser_callback", wysiwygmagebaymarketproduct_form_description.openFileBrowser);
                //
                });
					</script>
					';
		}else if($meta['arguments']['data']['config']['code'] == 'short_description'){
			$meta['arguments']['data']['config']['content'] = '<textarea data-ui-id="form-element-short_description" class=" textarea admin__control-textarea" cols="15" rows="20" name="short_description" id="magebaymarketproduct_form_short_description"></textarea>'
					.'<script type="text/javascript">					
						//
						window.tinyMCE_GZ = window.tinyMCE_GZ || {}; window.tinyMCE_GZ.loaded = true;
						require(["mage/adminhtml/wysiwyg/tiny_mce/setup"], function(){		  
		wysiwygmagebaymarketproduct_form_short_description = new tinyMceWysiwygSetup("magebaymarketproduct_form_short_description", {"enabled":true,"hidden":false,"use_container":false,"add_variables":false,"add_widgets":false,"no_display":false,"encode_directives":true,"baseStaticUrl":"","baseStaticDefaultUrl":"","directives_url":"","popup_css":"","content_css":"","width":"100%","height":"500px","plugins":[],"directives_url_quoted":"","add_images":true,"files_browser_window_url":"","files_browser_window_width":"1000","files_browser_window_height":"600"});wysiwygmagebaymarketproduct_form_short_description.setup("exact");
							//editorFormValidationHandler = wysiwygmagebaymarketproduct_form_short_description.onFormValidation.bind(wysiwygmagebaymarketproduct_form_short_description);
						   // Event.observe("toggleproduct_form_short_description", "click", wysiwygmagebaymarketproduct_form_short_description.toggle.bind(wysiwygmagebaymarketproduct_form_short_description));
							//varienGlobalEvents.attachEventHandler("formSubmit", editorFormValidationHandler);
							//varienGlobalEvents.clearEventHandlers("open_browser_callback");
							//varienGlobalEvents.attachEventHandler("open_browser_callback", wysiwygmagebaymarketproduct_form_short_description.openFileBrowser);
						//
						});
							</script>
							';			
