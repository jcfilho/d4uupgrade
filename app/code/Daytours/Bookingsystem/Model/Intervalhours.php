...
namespace Daytours\Bookingsystem\Model;
...
class Intervalhours extends \Magebay\Bookingsystem\Model\Intervalhours
{
    /**
   ...
   /**
	...
	function getIntervals($bookingId,$strDate = '',$status = 0,$calendarNumber = 1)
	{
		...
		if($strDate != '')
		{
			$collection->addFieldToFilter('intervalhours_booking_id',$bookingId)
                    ...
                    ->addFieldToFilter('calendar_number',$calendarNumber);
            if($status > 0)
            {
        ...
        $collection->addFieldToFilter('calendar_number',$calendarNumber);
		$dataIntervals = array();
		if(count($collection))
		{
     ...
     * @return array
     * */
    function getBaseTimeSlots($bookingId,$checkIn,$checkOut,$arSelect = array('*'),$calendarNumber = 1)
    {
        $collection = $this->getCollection()
     ...
     * @return array
     * */
    function getBaseTimeSlotsTwo($bookingId,$checkIn,$checkOut,$arSelect = array('*'))
    {
        $collection = $this->getCollection()
            ->addFieldToSelect($arSelect)
            ->addFieldToFilter('intervalhours_booking_id',$bookingId);
	...
	*/
	 ...
	 * @return $this
	 * */
	function  saveTimeSlots($bookingId, $params,$calendarId,$newCheckIn,$newCheckOut,$bkStore = true,$calendar_number = 1)
    {
        $totalQty = 0;
        $minPrice = 0;
        $minSpecialPrice = 0;
        ...
        if(count($params))
        {
            ...
            $currentCollection = $this->getCollection()
                    ...
                    ->addFieldToFilter('intervalhours_booking_id',$bookingId)
                    ->addFieldToFilter('calendar_number',$calendar_number);
            if($startDate != '')
            {
            ...
            foreach ($params as $param)
            {
                ...
                if($param['start_time'] != '' && $param['end_time'] != '')
                {
                    ...
                    if($i == 0)
                    {
                        ...
                        $minSpecialPrice = '';
                        if( $param['intervalhours_special_price'] != '' && $param['intervalhours_special_price'] > 0 ){
                            $minSpecialPrice = $param['intervalhours_special_price'];
                        }else{
                            unset($param['intervalhours_special_price']);
                        }
                ...
                $param['calendar_number'] = $calendar_number;
                //print_r($param);
        ...
        return array('min_price'=>$minPrice,'total_qty'=>$totalQty,'min_special_price'=>$minSpecialPrice);
    }
}