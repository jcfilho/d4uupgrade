...
class CalendarIntervals extends \Magento\Framework\App\Action\Action
{
    ...
    */
    ...
    public function execute()
    {
		...
		$arCalendars = array();
		$arrayseletct = array('*');
		$conditions = array('calendar_booking_type'=>$bookingType,'calendar_number' => $calendarNumber);
        ...
        if( $calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_DEFAULT ){
        ...
        }elseif ($calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_SECOND){
            $arTimeSlotsSelect = array('intervalhours_quantity','intervalhours_check_in','intervalhours_check_out','intervalhours_days');
            if(count($calendars))
            {
                if(count($calendars))
                ...
                foreach($dataCalendar as $mKey => $value)
                {
                    ...
                    $intervals = $intervalModel->getBaseTimeSlotsTwo($bookingId,$value['start_date'],$value['end_date'],$arTimeSlotsSelect);
                    $tempQty = 0;
                    if(count($intervals))
                    {
                        $dataCalendar[$mKey]['inter_value'] = $intervals->getData();
            ...
            //get array booking in cart
            $cartItems = $this->_bkOrderHelper->getArrayIntervalItemIncartTwo($bookingId,$itemId);
            $newArCartItems = array();
            $j = 0;
            foreach($cartItems as $cartItem)
            {
                $tempQty = 0;
                if(count($cartItem['hour_intervals_two']))
                {
                    foreach($cartItem['hour_intervals_two'] as $tempInter)
                    {
                        $tempQty += $cartItem['qty'];
                    }
                }
                $newArCartItems[$j]['check_in_two'] = $cartItem['check_in_two'];
                $newArCartItems[$j]['check_out_two'] =  $cartItem['check_in_two'];
                $newArCartItems[$j]['qty'] = $tempQty;
                $j++;
            }
            $bkOrderModel = $this->_bookingordersFactory->create();
            $arSelectOrder = array('bkorder_check_in_two','bkorder_check_out_two','bkorder_qty','bkorder_quantity_interval','bkorder_interval_time','bkorder_order_id');
            $orders = $bkOrderModel->getCollection()
                ->addFieldToSelect($arSelectOrder)
                ->addFieldToFilter('bkorder_booking_id',$bookingId)
                ->addFieldToFilter('bkorder_room_id',0);
            ...
            if(count($orders))
            {
                ...
                foreach($orders as $order)
                {
                    ...
                    $arOrder[$i]['check_in_two'] = $order->getBkorderCheckInTwo();
                    $arOrder[$i]['check_out_two'] = $order->getBkorderCheckInTwo();
                    $arOrder[$i]['qty'] = $tempQty ;
                    $i++;
                }
            }
