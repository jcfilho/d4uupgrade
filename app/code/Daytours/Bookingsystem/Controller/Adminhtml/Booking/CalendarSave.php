	...
	public function execute()
	{
		...
		try {
		        if($bookingTime == 3)
                {
                    ...
                    {
                        $timeSlots = $this->_intervalhours->saveTimeSlots($bookingId,$paramsTimSlot,$calendarId,$newCheckIn,$newCheckOut,false,$params['calendar_number']);
                        $totalQty = $timeSlots['total_qty'];
                        $minPirce = $timeSlots['min_price'];
                        $minSpecialPirce = ($timeSlots['min_special_price'] != '' && $timeSlots['min_special_price'] != 0) ? $timeSlots['min_special_price'] : null;
                        if($totalQty > 0)
                        {
                            $data = array(
                                'calendar_qty'=>$totalQty,
                                'calendar_price'=>$minPirce,
                                'calendar_promo'=>$minSpecialPirce
                            );

                            $model->setData($data)->setId($calendarId)->save();
                    ...
                    else
                    {
                        ...
                        {
                            $timeSlots = $this->_intervalhours->saveTimeSlots($bookingId,$paramsTimSlot,$newCalendarId,$newCheckIn,$newCheckOut,false,$params['calendar_number']);
                            $totalQty = $timeSlots['total_qty'];
                            $minPirce = $timeSlots['min_price'];
                            $minSpecialPirce = ($timeSlots['min_special_price'] != '' && $timeSlots['min_special_price'] != 0) ? $timeSlots['min_special_price'] : null;
                            if($totalQty > 0)
                            {
                                ...
                                $model->setData($data)->setId($newCalendarId)->save();
