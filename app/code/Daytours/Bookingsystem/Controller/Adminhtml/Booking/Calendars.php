...
class Calendars extends Action
{
     ...
     * @param \Magebay\Bookingsystem\Helper\BkOrderHelper $bkOrderHelper
     * @param \Daytours\BookingLocked\Api\BookingLockedRepositoryInterface $bookingLockedRepository
     */
	...
	(
        ...
        \Daytours\BookingLocked\Api\BookingLockedRepositoryInterface $bookingLockedRepository
	)
	{
	...
	public function execute()
	{
        ...
        $numberCalendar = $this->_request->getParam('calendar_number','1');
        $conditions = array('calendar_booking_type'=>$bookingType,'calendar_number' => $numberCalendar);
		...
		if(count($dataCalendar))
		{
            ...
            foreach($dataCalendar as $mKey => $value)
            {
                ...
                $intervals = $intervalModel->getBaseTimeSlots($bookingId,$value['start_date'],$value['end_date'],$arTimeSlotsSelect,$numberCalendar);
                $tempQty = 0;
                if(count($intervals))
                {
        ...
        if($bookingType == 'per_day')
        {
            if($bookingTime == 3)
            {
                if( $numberCalendar == 1 ){
                    ...
                    $arOrder = $this->_bkOrderHelper->getArrayIntervalsInOrdersCalendarTwo($bookingId);
                }
            }
            else
            {
                if( $numberCalendar == 1 ){
                    ...
                    $arOrder = $this->_bkOrderHelper->getArrayItemsInOrderCalendarTwo($bookingId,0);
                }
            }
        ...
        $lockedDates = $this->bookingLockedRepository->getLockedByProductId($bookingId,$numberCalendar);

        $finalLockedDates = [];
        foreach ($lockedDates as $lockedDate){
            $finalLockedDates[] = $lockedDate->getLockedDate();
        }

        $arrayData['locked_dates'] = $finalLockedDates;
