...
class BkCustomOptions extends \Magebay\Bookingsystem\Helper\BkCustomOptions
{
	/**
     * @var PriceHelper
	 * */
...
//                                    );
							...
							elseif($booking->getBookingTime() == 2)
							{
                                ...
                                if(isset($params['service_start']) && isset($params['service_end'])){
                                    $serviceStart = $params['service_start'];
                                    $serviceEnd = $params['service_end'];
                                }else {
                ...
                $additionalOptions[] = array(
                    'label' => __('regular_services'),
                    'value' => $params['extra_options_regular_services'],
                );
				if($okAddCart)
				{
    ...
    function createExtractOptionsByTransfer($product,$params)
    {
        $additionalOptions = array();
        $enable = $this->_bkHelperDate->getFieldSetting('bookingsystem/setting/enable');
        $formatDate = $this->_bkHelperDate->getFieldSetting('bookingsystem/setting/format_date');
            ...
            if($booking && $booking->getId() && $booking->getTypeId() == 'booking' && isset($params['check_in']))
            {
                ...
                if($booking->getBookingType() == 'per_day')
                {
                    ...
                    if($booking->getBookingTime() == 1 || $booking->getBookingTime() == 2 || $booking->getBookingTime() == 4 || $booking->getBookingTime() == 5)
                    {
                        ...
                        {
                            $checkIn = $this->_bkHelperDate->convertFormatDate($params['check_in']);
                            $checkOut = $this->_bkHelperDate->convertFormatDate($params['check_in']);
                            $checkInTwo = '';
                            $checkOutTwo = '';
                            if(isset($params['isRoundTrip']) && $params['isRoundTrip'] == 1){
                                $checkInTwo = $this->_bkHelperDate->convertFormatDate($params['check_in_two']);
                                $checkOutTwo = $this->_bkHelperDate->convertFormatDate($params['check_in_two']);
                            }
                            ...
                            if($booking->getBookingTime() == 1 || $booking->getBookingTime() == 4 || $booking->getBookingTime() == 5)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(isset($params['isRoundTrip']) && $params['isRoundTrip'] == 1){
                                        $arPrice = $this->_bkSimplePriceHelper->getPriceBetweenDaysTwoCalendars($booking,$checkIn,$checkOut,$checkInTwo,$checkOutTwo,$qty,$itemId,array(),$oldOrderItemId,$params['isRoundTrip']);
                                    }else{
                                        $arPrice = $this->_bkSimplePriceHelper->getPriceBetweenDays($booking,$checkIn,$checkOut,$qty,$itemId,array(),$oldOrderItemId);
                                    }


                                    if(isset($params['isRoundTrip']) && $params['isRoundTrip'] == 0){
                                        $additionalOptions[] = array(
                                            'label' => __('Type transfer'),
                                            'value' => __('Going'),
                                        );
                                    }
                                    if(isset($params['isRoundTrip']) && $params['isRoundTrip'] == 1){
                                    ...
                                    if( isset($params['check_in_two']) ){
                                        if( !empty($params['check_in_two']) ){
                                            $additionalOptions[] = array(
                                                'label' => __('Date of departure'),
                                                'value' => $params['check_in_two'],
                                            );
                                        }
                                    }
...
//                                    );
                    ...
                    elseif($booking->getBookingTime() == 3)
                    {
                        ...
                        else
                        {
                            $checkIn = $this->_bkHelperDate->convertFormatDate($params['check_in']);
                            $checkInTwo = '';
                            if(isset($params['check_in_two']) && $params['check_in_two'] != ''){
                                $checkInTwo = $this->_bkHelperDate->convertFormatDate($params['check_in_two']);
                            }
                            if(isset($params['isRoundTrip']) && $params['isRoundTrip'] == 1){
                                $arPrice = $this->_intervalsPrice->getIntervalsHoursPriceTwoCalendar($booking,$checkIn,$checkInTwo,$qty,$intervalsHours,$intervalsHoursTwo,$itemId,array(),$oldOrderItemId,$params['isRoundTrip']);
                            }else{
                                $arPrice = $this->_intervalsPrice->getIntervalsHoursPrice($booking,$checkIn,$qty,$intervalsHours,$itemId,array(),$oldOrderItemId);
                            }
                            ...
                            else
                            {
                                ...
                                $strIntervals = $this->getInfoIntervals($booking,$intervalsHours,$typeTime,'');
                                $additionalOptions[] = array(
                                    'label' => __('Date of arrival'),
                                    'value' => $params['check_in'],
                                );
                                ...
                                if( isset($params['check_in_two']) ){
                                    if( !empty($params['check_in_two']) ){
                                        $strIntervalsTwo = $this->getInfoIntervals($booking,$intervalsHoursTwo,$typeTime,'');
                                        $additionalOptions[] = array(
                                            'label' => __('Date of departure'),
                                            'value' => $params['check_in_two'],
                                        );
	...
	function getInfoIntervals($booking,$intervalsHours,$typeTime,$typeTime2){
        $strIntervals = '';
        $i = 0;
        foreach($intervalsHours as $intervalId)
        {
