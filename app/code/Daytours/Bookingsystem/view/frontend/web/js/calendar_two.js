define([
    ...
    $.fn.DaytoursAnyBookingCalendarTwo = function(objData)
    {
        var Data = {
        ...
        methodsTwo = {
            ...
            parseData : function()
            {
                ...
                bkSimpleDate = Data['bk_simple_date'];
                defaultTourType = Data['default_tour_type'];
            ...
            parseCalendarData : function()
            {
                ...
                $.post(DataURL, {
                    ...
                    default_tour_type : defaultTourType,
                    ...
                    var checkOut = $('#check-out-two').val();
                    //get result booking
                    if($.trim(checkIn) != '' && $.trim(checkOut) != '')
                    {
                        ...
                        $.ajax({
                            ...
                            {
                                if(bookingTime == 5 && $('.bk-extract-price-person').length)
                                {
                                    $('.bk-extract-price-person').html(res.html_person);
                                }
                                $('.booking-results').html(res.html_result);
                                var tempCheckIn = $('#temp-check-in-two').val();
                                var tempCheckOut = $('#temp-check-out-two').val();
                                methodsTwo.selectedDays(tempCheckIn,tempCheckOut);
                ...
                else
                {
                    ...
                    if(dataOfDay['qty'] != '')
                    {
                        if(bookingOrdersTwo)
                        {
                            ...
                            {
                                if(bookingOrdersTwo[key_order]['check_in_two'] != 'undefined' && bookingOrders[key_order]['check_out_two'])
                                {
            ...
            {
                //$('.item-two .booking-item-day').off('click');
                $('body .item-two').on('click','.booking-item-day',function(){
                    ...
                    $('#temp-check-out-two').val(endDate);
                    ...
                    $('#check-out-two').val(endDate1);
                    if(bkSimpleDate == 2)
                    {
                        $('#bk-checkin-checkout-two').val(startDate1 +' - '+ endDate1);
                    }
                    else
                    {
                ...
                if(bkSimpleDate == 2)
                {
                    $( ".item-two .booking-item-day" ).hover(
                        function() {
                            var strDayId = $(this).attr('id');
                            var arDayId = strDayId.split('_');
                            var strDateId = arDayId[1];
            ...
            selectedDays: function (date1,date2)
            {
                ...
                var strDay = objDate.getDate() >= 10 ? objDate.getDate() : '0'+objDate.getDate();
                var strDate = objDate.getFullYear()+'-'+month+'-'+strDay;
                if($('.item-two #0_'+strDate).length)
                {
                    $('.item-two #0_'+strDate).addClass('day-selected');
                }
                if($('.item-two #2_'+strDate).length)
                {
                    $('.item-two #2_'+strDate).addClass('day-selected');
                }
                if($('.item-two #1_'+strDate).length)
                {
                    $('.item-two #1_'+strDate).addClass('day-selected');
                }
                //day1 += oneDay;
                //}
            }
