...
class Intervals extends \Magebay\Bookingsystem\Block\Intervals
{
    /**
     ...
     * @param IntervalhoursFactory $intervalhoursFactory
	...
	function getBookingRequest()
	{
		...
		else
		{
            ...
            if(isset($params['check-in-two']))
            {
                //if page search
                $checkInTwo = date($formatDate,strtotime($params['check-in-two']));
                $tempCheckInTwo = $params['check-in-teo'];
            }
			$request = array(
				'check_in'=>$checkIn,
	...
	function getBookingResultTwoCalendars()
    {
        ...
        if(count($params))
        {
            ...
            if( isset($params['isRoundTrip']) ){
                ...
                if(isset($params['intervals_hours_two']))
                {
                    $intervalsHoursTwo = $params['intervals_hours_two'];
                }
            }
        ...
        else
        {
            ...
            if( isset($params['isRoundTrip']) ){
                if( $params['isRoundTrip'] ) {
                    $arPrices = $this->_intervalsPrice->getIntervalsHoursPriceTwoCalendar($booking,$checkIn,$checkInTwo,$qty,$intervalsHours,$intervalsHoursTwo,$itemId,$paramAddons);
                }else{
	...
	*/
	...
	function getListIntervals()
	{
        ...
        $intervals = array();
        $priceOfDay = $this->_rentPrice->getPriceOfDay($bookingId,$strDay,'per_day',$calendarNumber);
        if(isset($priceOfDay['price']) && $priceOfDay['price'] > 0)
        {
            $intervals = $intervalModel->getIntervals($bookingId,$strDay,1,$calendarNumber);
        }
        ...
        if( $calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_SECOND ){
            $cartItem =  $this->_bkOrderHelper->getCurrentIntervalItemIncartTwo($bookingId,$itemId);
        }else{
		...
		if(count($intervals))
		{
          ...
          /*  echo '<pre>';
			...
			foreach($intervals as $interval)
			{
                ...
                if( $calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_SECOND ){
                    //get total qty in $cart calendar two
                    $interOrdertotal = $this->_bkOrderHelper->getOrderIntervalsTotalTwo($bookingId,$strDay,$intervalsHours);
                }else{
                    //get total qty in $cart calendar one
                    $interOrdertotal = $this->_bkOrderHelper->getOrderIntervalsTotal($bookingId,$strDay,$intervalsHours);
                }

                if( $calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_SECOND ){
                    //get total qty in $cart calendar two
                    $totalQtyInCart = $this->_bkOrderHelper->getTotalInterItemInCartTwo($bookingId,$strDay,$intervalsHours,$itemId);
                }else{
                    //get total qty in $cart calendar one
				...
				{
                    if( $calendarNumber == \Daytours\Bookingsystem\Block\Transfer::CALENDAR_NUMBER_BY_SECOND ){
                        if($cartItem['check_in_two'] == $strDay && in_array($interval['intervalhours_id'],$cartItem['hour_intervals_two']))
                        {
        ...
        $module = $this->getRequest()->getModuleName();
        $resultWishlist = $this->loadDataIfComeFromEditWishlist($module,'Y-m-d');
        if( count($resultWishlist) > 0 ){
            $arrayData = array_merge($arrayData,$resultWishlist);
        }
...
//                            }
                        ...
                        if($item['code'] == 'optionsbooking_interval_one'){
//                            $valueConverted = $this->separeateValues($item['value']);
                            $result['interval_one'] = $item['value'];
                        }
                        if($item['code'] == 'optionsbooking_interval_two'){
                            //$valueConverted = $this->separeateValues($item['value']);
                            $result['interval_two'] = $item['value'];
                        }
